...
public interface AvaliacaoMotoristaRepository extends JpaRepository<AvaliacaoMotorista, Integer> {
    ...
    @Query("select new " +
            " sptech.projetojpadtoquery.resposta.ResumoAvaliacoesMotoristaResponse(am.motorista.nome, am.motorista.id) " +
            " from AvaliacaoMotorista am where am.motorista.id = ?1 " +
            " group by am.motorista.id")
    Optional<List<ResumoAvaliacoesMotoristaResponse>> getAvaliacaoPorIdMotorista(int idMotorista);

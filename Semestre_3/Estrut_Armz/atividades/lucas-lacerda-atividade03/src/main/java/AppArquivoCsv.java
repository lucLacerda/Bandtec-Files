...
public class AppArquivoCsv {
    ...
    public static void buscarPorNome(ListaObj<Remedio> listaObj) {

        Scanner leitor = new Scanner(System.in);

        System.out.println("Vamos procurar pelo seu remédio:");

        System.out.println("Digite o nome dele:");
        String nome = leitor.next();

        for (int i = 0; i < listaObj.getTamanho(); i++) {
            if (listaObj.getElemento(i).getNome().toLowerCase().equals(nome.toLowerCase())){
